...
def adb_install():
    if worker.state() != QProcess.Running:
        try:
            ...
            dialog("WARNING",adbinstall.command, "")
            if window.yesno:
                worker.start(adb, ["install", file])
            else:
                pass
        except:
            messagebox("Please select a valid file first!")
    else: 
        messagebox("Another process is running!")
...
def adb_sideload():
    if worker.state() != QProcess.Running:
        try:
            terminal.clear()
            file = os.path.join(workdir, zipslist.currentItem().text())
            dialog("WARNING",adbsideload.command, "")
            if window.yesno:
...
def flashing_unlock():
    if worker.state() != QProcess.Running:
        terminal.clear()
        dialog("WARNING",unlockblnew.command, "")
        if window.yesno:
...
def flashing_lock():
    if worker.state() != QProcess.Running:
        terminal.clear()
        dialog("WARNING",relockblnew.command, "")
        if window.yesno:
...
def oem_unlock():
    if worker.state() != QProcess.Running:
        terminal.clear()
        dialog("WARNING",unlockblold.command, "")
        if window.yesno:
...
def oem_lock():
    if worker.state() != QProcess.Running:
        terminal.clear()
        dialog("WARNING",relockblold.command, "")
        if window.yesno:
...
def boot_image():
    if worker.state() != QProcess.Running:
        try:
            file = os.path.join(workdir, zipslist.currentItem().text())
            terminal.clear()
            dialog("WARNING",bootimg.command, "")
            if window.yesno:
...
def vbmeta():
    if worker.state() != QProcess.Running:
        try:
            file = os.path.join(workdir, zipslist.currentItem().text())
            terminal.clear()
            dialog("WARNING",flashvbmeta_disable.command, "")
            if window.yesno:
...
def erase_userdata():
    if worker.state() != QProcess.Running:
        terminal.clear()
        dialog("WARNING",formatuserdata.command, "")
        if window.yesno:
...
githubpagebut.setIcon(QIcon(r"C:\AFS-Windows\assets\github.png"))
githubpagebut.clicked.connect(githubopen)
tiktokpagebut = QPushButton("TikTok page")
tiktokpagebut.setIcon(QIcon(r"C:\AFS-Windows\assets\tiktok.png"))
colorsbut(tiktokpagebut,"#00000000","#3c3c3c37")
colorsbut(githubpagebut,"#00000000","#3c3c3c37")
...
flash = Commands("Flash image to partition","Flash",flash_image,False,["boot","recovery","system","vendor","vbmeta","userdata","super"],"fastboot flash [selected partition] [selected file]","Type valid partition name")
